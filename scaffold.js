...
const getModelNameFromFile = text => {
    const modelName = text.split('TxQuery.model(')[1].split(',')[0].split('"').join('').trim()
    return modelName
}
...
const smallModelName = modelName.charAt(0).toLowerCase() + modelName.substring(1)

const templateFile = fs.readFileSync(`./Templates/${template}.js`, 'utf8')
let newFile = templateFile

const customVariables = Object.fromEntries(args.filter(x => x.includes(":")).map(x => x.split(":")))
const fileVariables = Object.fromEntries([...newFile.matchAll("<<(.*?)>>")].map(([_, varName]) => varName).map(v => [v, 'null']))
...
Object.entries(data).forEach(([k, v]) => newFile = newFile.split(`<<${k}>>`).join(v))
...
if(!allRoutes.includes(`require("../Controllers/${controllerName}.js")`))
    allRoutes = `const express = require("express");\nconst ${controllerName} = require("../Controllers/${controllerName}.js");\n` + allRoutes.replace('const express = require("express")', "")

if(!allRoutes.includes(`router.post('/${smallModelName}', ${controllerName}.store)`)) allRoutes += `\n\n
